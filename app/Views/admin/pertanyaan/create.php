...
<h1 class="mt-4">Tambah Pertanyaan untuk Kuesioner: "<?= esc($kuesioner['nama_kuesioner']) ?>"</h1>
    ...
    <div class="card-body">
        ...
        <form action="<?= base_url('admin/pertanyaan/store') ?>" method="post">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control <?= $validation && $validation->hasError('teks_pertanyaan') ? 'is-invalid' : '' ?>" id="teks_pertanyaan" name="teks_pertanyaan" rows="3" required><?= old('teks_pertanyaan') ?></textarea>
            ...
            <div class="mb-3">
                ...
                <select class="form-select <?= $validation && $validation->hasError('jenis_jawaban') ? 'is-invalid' : '' ?>" id="jenis_jawaban" name="jenis_jawaban" required>
                    ...
                    <option value="skala" <?= old('jenis_jawaban') == 'skala' ? 'selected' : '' ?>>Skala (misal: 1-5)</option>
                    <option value="pilihan_ganda" <?= old('jenis_jawaban') == 'pilihan_ganda' ? 'selected' : '' ?>>Pilihan Ganda</option>
                    <option value="isian" <?= old('jenis_jawaban') == 'isian' ? 'selected' : '' ?>>Isian Singkat/Panjang</option>
                </select>
            ...
            <div id="opsi_jawaban_container" class="mb-3" style="display: none;">
                ...
                <div id="opsi_list">
                    <?php
                    // Ambil opsi yang sudah diisi sebelumnya (jika ada error validasi)
                    $old_opsi_teks = old('opsi_teks');
                    $old_opsi_nilai = old('opsi_nilai');

                    if (!empty($old_opsi_teks)) :
                        foreach ($old_opsi_teks as $key => $teks) :
                    ?>
                            <div class="input-group mb-2 opsi-item">
                                ...
                                <input type="number" name="opsi_nilai[]" class="form-control opsi-nilai-input <?= $validation && $validation->hasError('opsi_nilai.' . $key) ? 'is-invalid' : '' ?>" placeholder="Nilai (1-5)" min="1" max="5" value="<?= esc($old_opsi_nilai[$key] ?? '') ?>" style="display: none;">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="urutan" id="urutan" class="form-control <?= $validation && $validation->hasError('urutan') ? 'is-invalid' : '' ?>" value="<?= old('urutan') ?>" min="0" required>
